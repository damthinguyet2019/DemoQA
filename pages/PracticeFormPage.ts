...
export class PracticeFormPage {
  ...
  rdGenderAndHobies: string =
    'xpath=//label[text()="@param"]//preceding-sibling::input';
  readonly txtPicture;
  ...
  async inputData(
    ...
    const picturePath: string = process.cwd() + "/testcase/data/" + picture; // thu muc chua du an
  ...
  async inputDateOfBirth(dateOfBirth: string) {
    let dateOfBirths = dateOfBirth.split(" "); // ngay nhap vao chưa biết ngày nào
    const day: string = dateOfBirths[0] || "";  // xác định được là hằng số thì để const
