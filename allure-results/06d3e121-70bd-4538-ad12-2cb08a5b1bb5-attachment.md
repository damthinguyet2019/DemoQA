...
```
Error: locator.isVisible: Error: strict mode violation: locator('//span[@class=\'rc-tree-switcher rc-tree-switcher_close\']') resolved to 3 elements:
...
Call log:
    - checking visibility of locator('//span[@class=\'rc-tree-switcher rc-tree-switcher_close\']')
...
> 14 |         const isExpanded: boolean = await checkBoxPage.expandAllButton.isVisible();
  ...
  19 |         const isExpanded: boolean = await checkBoxPage.checkBoxList.isVisible();
  20 |         await expect(isExpanded).toBeFalsy();
  21 |     }
  22 |     );
  23 |     test("TC03-Click on checkbox", async () => {
  24 |         await checkBoxPage.checkBoxListInput.click();
  25 |         const isChecked: boolean = await checkBoxPage.checkBoxListInput.isChecked();
