...
test.describe('Practice Form Test', () => {
  ...
  const dateOfBirth: string = '01 Jannuary 1980';
  ...
  await practiceFormPage.inputData(firstName, lastName, email, gender, mobile, dateOfBirth, subjects, hobbies, picture, currentAddress, state,city);
  ...
  const actualStudentEmail: string = await thanksForSubmittingPage.getValueByLabel(thanksForSubmittingPage.lblValue, 'Email');
...
const actualPicture: string = await thanksForSubmittingPage.getValueByLabel(thanksForSubmittingPage.lblValue, 'Picture');
    ...
    const actualAdress: string = await thanksForSubmittingPage.getValueByLabel(thanksForSubmittingPage.lblValue, 'Current Adress');
    expect (actualAdress).toBe(currentAddress) // truyen thang bien da nhap tu ben tren 

    const actualStateAndCity: string = await thanksForSubmittingPage.getValueByLabel(thanksForSubmittingPage.lblValue, 'State and City');
